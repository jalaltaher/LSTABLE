...
def plot_stable_density_histogram(alpha: float,P:float, Q:float,A:float,B:float, plot_grid: np.ndarray, nb_sample: int, nb_bins: int,c: float = 0, density=True, histogram=True):
    ...
    if histogram==True:
        ...
        values,bins=np.histogram(sample_array,density=True,bins=nb_bins)#histogram_zoom(sample_array, minimum_bound,maximum_bound,nb_bins)
